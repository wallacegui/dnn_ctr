...
import pandas as pd
import keras
from keras.layers import Dense, Input, LSTM, Embedding, Dropout, Activation, Reshape, Lambda, concatenate, Dot,dot, add, Masking,TimeDistributed
...
class DeepCross:
    def __init__(self,num_features,sparse_features,features_num_dict,k=10,cross=3,hidden_layer=[256,128,64],optimizer=Adam(0.001)):
    ...
    def create_model(self):
        ...
        #######dnn layer##########
        x0 =  concatenate(nums_input+sparse_embedding, axis=-1)
        ####### deep layers #######
        fc_input_layer = x0
        ...
        for i in range(4):
            ...
            xl = add([xl, tmp])
        ########linear layer##########
        lr_input_layer = concatenate([xl,fc_input_layer], axis=-1)
        lr_layer = Dense(1)(lr_input_layer)
